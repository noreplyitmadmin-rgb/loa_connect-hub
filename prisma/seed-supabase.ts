...
async function main() {
  ...
  const { data: admin, error: adminErr } = await supabase
    .from("users")
    .insert({ name: "Dr. Admin", email: "admin@econsult.com", passwordHash, role: "ADMIN", hasLoggedInBefore: true })
    .select("*")
    .single()
  ...
  const { data: dean, error: deanErr } = await supabase
    .from("users")
    .insert({ name: "Regie Ellana", email: "regie@itmlyceumalabang.onmicrosoft.com", passwordHash, role: "DEAN" })
    .select("*")
    .single()
  ...
  const { data: faculty1, error: facErr } = await supabase
    .from("users")
    .insert({ name: "Nin Alamo", email: "nino_francisco_alamo@itmlyceumalabang.onmicrosoft.com", passwordHash, role: "FACULTY", departmentId: department.id })
    .select("*")
    .single()
  ...
  const { data: student1, error: stuErr } = await supabase
    .from("users")
    .insert({ name: "Nino Francisco Alamo", email: "nin.alamo@outlook.com", passwordHash, role: "STUDENT" })
    .select("*")
    .single()
