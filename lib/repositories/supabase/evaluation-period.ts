...
export const evaluationPeriodRepository: IEvaluationPeriodRepository = {
  async list(filter) {
    ...
    if (filter?.schoolYear) q = q.eq("schoolYear", filter.schoolYear)
  ...
  async findActive() {
    const { data, error } = await supabase.from("evaluation_periods").select("*").eq("isActive", true).single()
    if (error) {
      if (error.code === "PGRST116") return null
      throw error
    }
  ...
  async create(input: CreateEvaluationPeriodInput) {
