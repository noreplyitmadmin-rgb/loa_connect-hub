    ...
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 sm:py-2 rounded-lg border border-slate-200 text-slate-700 text-xs font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 min-h-[44px]"
            >
      ...
      {pendingRequests.length > 0 && (
        <div className="animate-fade-in" style={{ animationDelay: "0.12s" }}>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-sm font-bold text-slate-800">Action Required</span>
          ...
          <div className="space-y-2">
            {pendingRequests.map((apt) => (
              <div
                key={apt.id}
                className="card p-4 sm:p-5 bg-white border-l-4 border-l-amber-400 hover:shadow-md transition-shadow duration-200"
              >
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {apt.title && (
                        <p className="text-xs text-slate-600 font-medium mt-0.5 truncate">{apt.title}</p>
                      )}
                      <span className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider ${
                    ...
                    {acceptingId === apt.id ? (
                      <div className="flex flex-col sm:flex-row gap-2 w-full sm:items-center">
                        <input
                          ...
                          className="w-full sm:w-64 px-3 py-2.5 sm:py-2 rounded-lg border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-gold-500/40 focus:border-gold-400"
                        />
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="flex-1 sm:flex-none inline-flex items-center justify-center px-4 py-2.5 sm:py-2 rounded-lg border border-slate-200 text-slate-600 text-xs font-semibold hover:bg-slate-50 transition-colors min-h-[44px] sm:min-h-0"
                          >
                            Cancel
                          </button>
                        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 animate-fade-in" style={{ animationDelay: "0.28s" }}>
        ...
        <div className="card p-5 sm:p-6 bg-white">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">This Week</p>
          <div className="flex gap-2 sm:gap-3">
            {weeklyPreview.map((day, i) => (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">{DAY_LABELS[i]}</span>
                <div
        ...
        {recentActivity.length > 0 && (
          <div className="card p-5 sm:p-6 bg-white">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Recent Activity</p>
            <div className="space-y-2">
              {recentActivity.map((apt) => (
                <div key={apt.id} className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-slate-400">{apt.date} · {apt.startTime}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="card p-5 sm:p-6 bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/60 dark:from-amber-950/30 dark:to-amber-900/20 dark:border-amber-800/30">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-amber-100">Department Pulse</p>
                  <p className="text-xs text-slate-500 dark:text-amber-300/70">{departmentName || "Your Department"}</p>
                </div>
              </div>
              <Link
                href="/admin/reports/health"
                className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 sm:py-2 rounded-lg bg-white border border-amber-200 text-amber-700 text-xs font-semibold hover:bg-amber-50 hover:border-amber-300 transition-all duration-200 min-h-[44px] sm:min-h-0 dark:bg-amber-900/20 dark:border-amber-700/30 dark:text-amber-300 dark:hover:bg-amber-900/40 dark:hover:border-amber-600/50"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={s.label} className="bg-white/70 rounded-lg p-3 border border-amber-100/50 dark:bg-amber-950/40 dark:border-amber-800/30">
                  <p className="text-lg font-bold text-slate-900 dark:text-amber-100">{s.value}</p>
                  <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mt-0.5 dark:text-amber-300/70">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {appointments.length === 0 && (
        <div className="card p-12 sm:p-16 bg-white text-center animate-fade-in" style={{ animationDelay: "0.15s" }}>
          ...
          <h2 className="text-lg font-bold text-slate-900 mb-2">No meetings yet</h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <Link
              href="/faculty/availability"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg border border-slate-200 text-slate-700 font-semibold text-sm hover:bg-slate-50 transition-all duration-200"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
