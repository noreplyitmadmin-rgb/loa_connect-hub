        ...
        <div className="space-y-4">
              ...
              {!csvRows && !csvImportResult && (
                <div className="space-y-4">
                  <div
                    onClick={() => csvFileRef.current?.click()}
                    className="flex flex-col items-center justify-center gap-4 p-12 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-3xl bg-surface-dim/30 hover:bg-surface-dim/60 cursor-pointer transition-all"
                  >
                    <div className="w-16 h-16 rounded-full bg-gold-100 dark:bg-gold-900/40 flex items-center justify-center">
                      <svg className="w-8 h-8 text-gold-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                      </svg>
                    </div>
                    <p className="text-base font-semibold text-secondary">Tap to choose a CSV file</p>
                    <p className="text-sm text-tertiary">Headers: <code className="bg-surface-dim px-2 py-1 rounded text-xs">{TEMPLATE_HEADERS}</code></p>
                    <input
                      ref={csvFileRef}
                      type="file"
                      accept=".csv"
