...
export default function StandaloneEvaluationPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!evalRes.ok) { window.close(); return }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setExistingComment(comment || null)
    } catch {
  ...
  // ── Thank you / submitted view ──
  if (isSubmitted) {
    const labelMap = ["", "Poor", "Fair", "Good", "Very Good", "Excellent"]
    const canClose = typeof window !== "undefined" && (window.history.length === 1 || window.opener)
    return (
      <div className="min-h-dvh bg-surface-muted pt-20 pb-12 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto text-center mb-8">
          ...
          <p className="text-sm text-tertiary leading-relaxed">
            Your evaluation for <strong className="text-primary">{evaluateeName}</strong> has been submitted successfully.
          </p>
          {submittedAt && (
            <p className="text-xs text-tertiary mt-2">
              Submitted {new Date(submittedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}
            </p>
          )}
        </div>
        ...
        {showScrollTop && (
          <button
            ...
            className="fixed bottom-6 right-6 z-60 w-12 h-12 rounded-full bg-brand-600 text-white shadow-lg hover:bg-brand-700 active:scale-95 transition-all flex items-center justify-center"
