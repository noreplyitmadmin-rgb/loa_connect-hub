...
export async function POST(request: NextRequest) {
  ...
  let importRows: { email: string; name: string; sectionName: string; sectionProgram: string }[]
  ...
  if (contentType.includes("application/json")) {
    ...
    const rawRows = body.rows as { email: string; name?: string; section: string }[] | undefined
    if (!rawRows || !Array.isArray(rawRows) || rawRows.length === 0) {
      return NextResponse.json({ error: "Rows array is required" }, { status: 400 })
    }
    importRows = rawRows.map((r, _i) => {
      const { program, name: sectionName } = parseSectionIdentifier(r.section || "")
      return { email: r.email.toLowerCase().trim(), name: r.name || "", sectionName, sectionProgram: program }
    })
  } else {
