...
export async function GET(
  ...
  try {
    ...
    // Verify dean has access to this department
    ...
    const { data: deptAccess } = await supabase
      ...
      .eq("deanId", deanId)
      .single()
    if (!deptAccess) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    ...
    console.error("Consolidated dean faculty evaluation error:", e)
